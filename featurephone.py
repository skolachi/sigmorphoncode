...
def phone_to_features(ph):
    ...
    [('vls', 'alv', 'stp', 'frc'), ('hgh', 'fnt', 'unr', 'vwl','primary stress'), ('vcd', 'alv', 'frc')]

    """
    if ph[-1] == '0':
        return list(phone_feature_map[ph[:-1]]) + ['no stress']
    if ph[-1] == '1':
        return list(phone_feature_map[ph[:-1]]) + ['primary stress']
    if ph[-1] == '2':
        return list(phone_feature_map[ph[:-1]]) + ['secondary stress']
    return list(phone_feature_map[ph])
